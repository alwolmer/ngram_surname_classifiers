...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Scrape Delegacia do Estrangeiro records by nationality."
    )
    ...
    parser.add_argument(
        "--resume-all",
        action="store_true",
        help="Resume from all .csv files in current directory except falhas_scraping*",
    )
...
def get_proxy():
    return (
        {"http": random.choice(PROXIES), "https": random.choice(PROXIES)}
        if PROXIES
        else None
    )
...
def fetch_page(nacionalidade, page):
    ...
    headers = get_headers()
    proxy = get_proxy()
    data = {"frm": build_form(nacionalidade), "page": page, "limit": 10}
    response = session.post(
        BASE_URL, headers=headers, data=data, proxies=proxy, timeout=20
    )
    response.raise_for_status()
...
def fetch_page_safe(nacionalidade, page):
    time.sleep(
        random.uniform(*POLITE_DELAY_RANGE)
    )  # single delay before initial request
    try:
...
def main(
    targets, list_only=False, scrape_only=False, resume_from=None, resume_all=False
):
    nacionalidade_info = {}
